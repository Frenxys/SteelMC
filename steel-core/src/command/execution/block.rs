...
mod tests {
    ...
    #[test]
    fn block_input_applies_nbt_even_when_the_state_is_unchanged() {
        init_test_registry();
        init_behaviors();
        init_block_entities();
        let pos = BlockPos::new(4, 64, 4);
        let world = fresh_test_world("command_block_input_nbt");
        ...
        let mut reader = StringReader::new("furnace{lit_time_remaining:7s}");
        let input = parse_block_input(&mut reader).expect("furnace block input should parse");
        assert!(
            input
                .place(&world, pos, flags)
                .expect("owned command NBT should reborrow")
        );
        let Some(entity) = world.get_block_entity(pos) else {
            panic!("furnace placement should create a block entity");
        };
        assert_eq!(
            entity.save_without_metadata().short("lit_time_remaining"),
            Some(7)
        );

        let mut reader = StringReader::new("furnace{lit_time_remaining:9s}");
        let input = parse_block_input(&mut reader).expect("updated furnace input should parse");
        assert!(
            input
                .place(&world, pos, flags)
                .expect("owned command NBT should reborrow")
