...
impl PrimedTntEntity {
    ...
    pub fn primed(
        ...
        entity.set_velocity(DVec3::new(-angle.sin() * 0.02, 0.2, -angle.cos() * 0.02));
    ...
    #[must_use]
    pub fn get_random_short_fuse(fuse: i32) -> i32 {
...
impl Entity for PrimedTntEntity {
    ...
    fn restore_additional_references_from(&self, previous: &dyn Entity) {
        let Some(previous) = previous.downcast_ref::<Self>() else {
            return;
        };
        self.state.lock().owner = previous.owner_reference();
    }
...
mod tests {
    ...
    fn priming_applies_vanilla_motion_and_entity_properties() {
        ...
        assert!((velocity.x.hypot(velocity.z) - 0.02).abs() < f64::EPSILON * 16.0);
