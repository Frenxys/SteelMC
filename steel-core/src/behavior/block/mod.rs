...
pub trait BlockBehavior: Send + Sync {
    ...
    ///
    /// Implementations can override this for trigger-only interactions or
    /// source-specific destruction while retaining the shared explosion context.
    fn on_explosion_hit(
        &self,
        state: BlockStateId,
        world: &Arc<World>,
    ...
    /// Applies the default block response from an overriding explosion hook.
    ...
    /// Called after this block has been removed by an explosion.
    #[expect(
        unused_variables,
        reason = "default trait implementation ignores all params"
    )]
